...
-- Keep earliest canonical record when historical duplicates now merge cross-device.
WITH ranked AS (
  SELECT
    id,
    row_number() OVER (
...
SET ingestion_status = 'failed',
    error_message = coalesce(e.error_message, 'dedup_collision_after_cross_device_update')
FROM ranked r
WHERE e.id = r.id
  AND r.rn > 1;
